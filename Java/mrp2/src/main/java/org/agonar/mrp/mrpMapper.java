...
public class mrpMapper extends Mapper<LongWritable, Text, Text, IntWritable> {
	...
	public void map(LongWritable key, Text value, Context context) throws IOException, InterruptedException {
		...
		//identify words, contractions, urls, and hashtags
		String regex = "http[s]?://(?:[a-zA-Z]|[0-9]|[$-_@.&+]|[!*\\(\\),]|(?:%[0-9a-fA-F][0-9a-fA-F]))+|(\\B(\\#[a-zA-Z]+\\b)(?!;)|(?!'.*')\\b[\\w']+\\b)";   
		...
		while(m.find()) {               	   
			...
			for(int j=0; j<stopwords.length;j++) {
				if(m.group().equalsIgnoreCase(stopwords[j])) {
					match=true;
					break;
				}
			} 
			if(!match) context.write(new Text(m.group()), new IntWritable(1));
		}
